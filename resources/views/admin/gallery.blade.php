            ...
            <div class="flex-none w-full max-w-full px-3">
                <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
                    <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent flex justify-between">
                        <h6>Gallery table</h6>
                        <button onclick="showPopup('tambah-berita')" class="cursor-pointer flex p-2 m-1 rounded-md items-center bg-[#1410EB] hover:bg-blue-700 text-white font-medium ">
                                    ...
                                    <tr class="">
                                        <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                            <div class="px-4 text-ellipsis overflow-hidden max-w-xs">
                                                <h6 class="mb-0 text-base leading-normal">{{$data->judul}}</h6>
                                            </div>
                                        </td>
                                        <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                            <div class="max-w-sm">
                                                <h6 class="mb-0 text-sm leading-normal break-words line-clamp-3">{{$data->isi}}</h6>
                                            </div>
                                        </td>
                                        <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                            <div class="px-4">
                                                <h6 class="mb-0 text-sm leading-normal">{{$data->tanggal}}</h6>
                                            </div>
                                        </td>
                                        <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                            <div class="px-4">
                                                <h6 class="mb-0 text-sm leading-normal">{{ $data->kategori->nama}}</h6>
                                            </div>
                                        </td>
                                        <td class="p-2 text-sm leading-normal text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                            <div class="px-4 flex justify-center">
                                                @if ($data->gambar != null)
                                                <img src="{{ asset('/storage/uploads/'.$data->gambar)}}" class="w-full" alt="">
                                                @else
                                                <h6 class="mb-0 text-base leading-normal">Tidak ada Gambar</h6>
                                                @endif
                                            </div>
                                        </td>
                                        <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                            <div class="flex justify-center">
                                                <a href="{{ route('berita.edit', $data->id) }}" class="cursor-pointer flex p-2 m-1 rounded-md items-center bg-[#F96D0E] hover:bg-orange-600 text-white font-medium ">
                                                    <i class="fa fa-pencil pr-1" aria-hidden="true"></i> Edit
                                                </a>
                                                <form id="deleteForm{{ $data->id }}" action="{{ route('berita.destroy', $data->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
...
<div id="tambah-berita" class="fixed inset-0 hidden items-center justify-center bg-black bg-opacity-50">
    <div
        ...
        <form action="{{ route('berita.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="p-6">
                ...
                <div class="mb-5">
                    <label for="judul" class="block mb-2 text-base font-medium text-gray-900 ">Masukan Kategori</label>
                ...
                <div class="mb-5">
                    <label for="tanggal" class="block mb-2 text-base font-medium text-gray-900 ">Masukan Tanggal</label>
                    <input type="date" id="tanggal" name="tanggal" class="bg-gray-50 border border-gray-300 text-gray-900 text-base rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
                </div>
                <div class="mb-5">
