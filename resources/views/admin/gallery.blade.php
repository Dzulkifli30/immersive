    ...
    <nav class="relative flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" navbar-main navbar-scroll="true">
        <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
            <nav>
                ...
                <h6 class="mb-0 font-bold capitalize">Gallery table</h6>
            </nav>
            ...
            <div class="flex-none w-full max-w-full px-3">
                <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
                    <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent flex justify-between">
                        <h6>Gallery table</h6>
                        <button onclick="showPopup('tambah-gallery')" class="cursor-pointer flex p-2 m-1 rounded-md items-center bg-[#1410EB] hover:bg-blue-700 text-white font-medium ">
                            <i class="fa fa-plus-circle pr-1" aria-hidden="true"></i> Tambah Gallery
                        </button>
                    </div>
                                    ...
                                    <tr class="">
                                        <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                            <div class="px-4 text-ellipsis overflow-hidden max-w-xs">
                                                <h6 class="mb-0 text-base leading-normal">{{$data->customer}}</h6>
                                            </div>
                                        </td>
                                        <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                            <div class="max-w-sm">
                                                <h6 class="mb-0 text-sm leading-normal break-words line-clamp-3">{{$data->deskripsi}}</h6>
                                            </div>
                                        </td>
                                        <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                            <div class="flex justify-center">
                                                <a href="{{ route('gallerys.edit', $data->id) }}" class="cursor-pointer flex p-2 m-1 rounded-md items-center bg-[#F96D0E] hover:bg-orange-600 text-white font-medium ">
                                                    <i class="fa fa-pencil pr-1" aria-hidden="true"></i> Edit
                                                </a>
                                                <form id="deleteForm{{ $data->id }}" action="{{ route('gallerys.destroy', $data->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
...
<div id="tambah-gallery" class="fixed inset-0 hidden items-center justify-center bg-black bg-opacity-50">
    <div
        ...
        <div class="flex items-center justify-between p-3 px-6">
            <h3 class="text-xl font-bold">Tambah Gallery</h3>
            <button onclick="hidePopup('tambah-gallery')" class="text-black font-bold px-2 py-1 rounded bg-primary-red">X</button>
        </div>
        <form action="{{ route('gallerys.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="p-6">
                <div class="mb-5">
                    <label for="customer" class="block mb-2 text-base font-medium text-gray-900 ">Nama Customer</label>
                    <input type="text" id="customer" name="customer" class="bg-gray-50 border border-gray-300 text-gray-900 text-base rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
                </div>
                <div class="mb-5">
                    <label for="deskripsi" class="block mb-2 text-base font-medium text-gray-900 ">Deskripsi Event</label>
                    <textarea id="deskripsi" name="deskripsi" rows="4" class="block p-2.5 w-full text-base text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
