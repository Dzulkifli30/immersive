  ...
  <div class="w-full px-6 py-6 mx-auto">
    ...
    <div class="items-start p-8 space-y-4">
      <div class="flex justify-center">
        <h2 class="text-4xl uppercase font-semibold text-gray-900 mb-4 text-center">Header pada Home</h2>
        <i class="fa fa-pencil-square-o text-sm" aria-hidden="true"></i>
      </div>
      <div id="show" class="lg:w-2/3 mx-auto">
        <div class="bg-white lg:flex shadow border rounded-lg p-4">
          <div class="lg:w-1/2 w-full">
            <div class="mb-4 lg:w-11/12">
              <label for="judul" class="block text-gray-700 text-lg font-bold mb-2">Judul:</label>
              <p class="py-2 text-gray-700 leading-tight font-semibold"> {{ $header->judul }}</p>
            </div>
            <div class="mb-4 lg:w-11/12">
              <label for="subjudul" class="block text-gray-700 text-lg font-bold mb-2">Sub-judul:</label>
              <p class="py-2 text-gray-700 leading-tight font-semibold text-justify">{{ $header->subjudul }}</p>
            </div>
          </div>
          <div class="lg:w-1/2 w-full">
            <div class="mb-4">
              <label for="judul" class="block text-gray-700 text-lg font-bold mb-2">Gambar:</label>
              <div class="p-10">
                <img class="w-full" src="{{ asset('/storage/uploads/'.$header->gambar) }}" alt="">
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  toggleButton.addEventListener('click', function() {
    // Jika kotak biru terlihat, sembunyikan dan tampilkan kotak hijau
    if (!showBox.classList.contains('hidden')) {
      showBox.classList.add('hidden');
      formBox.classList.remove('hidden');
    } else {
      // Jika kotak hijau terlihat, sembunyikan dan tampilkan kotak biru
