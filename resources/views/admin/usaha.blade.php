        ...
        <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
          <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent flex justify-between">
            ...
            <button onclick="showPopup('tambah-usaha')" class="cursor-pointer flex p-2 m-1 rounded-md items-center bg-[#1410EB] hover:bg-blue-700 text-white font-medium ">
              <i class="fa fa-plus-circle pr-1" aria-hidden="true"></i> Tambah Usaha
            </button>
          </div>
            ...
            <div class="p-0 overflow-x-auto">
              <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                <thead class="align-bottom">
                  <tr>
                    <th class="px-6 py-3 font-bold text-left uppercase align-middle bg-transparent border-b border-gray-200 shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Bidang Usaha</th>
                ...
                <tbody>
                  ...
                  <tr>
                    <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                      <div class=" px-4 ">
                        <h6 class="mb-0 text-base leading-normal break-words">{{$data->bidang_usaha}}</h6>
                      </div>
                    </td>
                    <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                      <div class="flex justify-center">
                        <a href="{{ route('usaha.edit', $data->id) }}" class="cursor-pointer flex p-2 m-1 rounded-md items-center bg-[#F96D0E] hover:bg-orange-600 text-white font-medium ">
                          <i class="fa fa-pencil pr-1" aria-hidden="true"></i> Edit
                        </a>
                        <form id="deleteForm{{ $data->id }}" action="{{ route('usaha.destroy', $data->id) }}" method="POST">
                          @csrf
                          @method('DELETE')
...
<div id="tambah-usaha" class="fixed inset-0 hidden items-center justify-center bg-black bg-opacity-50">
  <div
    ...
    <div class="flex items-center justify-between p-3 px-6">
      <h3 class="text-xl font-bold">Tambah Bidang Usaha</h3>
    ...
    <form action="{{ route('usaha.store') }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="p-6">
        <div class="mb-5">
          <label for="bidang_usaha" class="block mb-2 text-base font-medium text-gray-900 ">Nama Bidang Usaha</label>
          <input type="text" id="bidang_usaha" name="bidang_usaha" class="bg-gray-50 border border-gray-300 text-gray-900 text-base rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required />
        </div>
