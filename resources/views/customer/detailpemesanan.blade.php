    ...
    <div class="w-full px-6 py-6 mx-auto">
        ...
        <div class="items-start p-8 space-y-4">
            <div class="flex justify-center">
                <h2 class="text-4xl uppercase font-semibold text-gray-900 mb-4 text-center">Detail Pemesanan</h2>
            </div>
                    ...
                    <div class="w-full">
                        <div class="flex w-full">
                            ...
                            <div class="mb-4 flex gap-2 w-1/2">
                                ...
                                <label for="nama_event" class="block text-gray-700 text-lg mb-2"> {{$pemesanan->nama_event}}</label>
                            </div>
                        </div>
                        <div class="flex w-full">
                            <div class="mb-4 flex gap-2 w-1/2">
                        ...
                        <div class="flex w-full">
                            <div class="mb-4 flex gap-2 w-1/2 items-center">
                                <label for="isi" class="block text-gray-700 text-lg font-bold mb-2">Status:</label>
                                @if ($pemesanan->status == 0)
                                <h6 class="mb-1 text-sm leading-normal font-bold text-yellow-300">Pending</h6>
                                @elseif ($pemesanan->status == 1)
                                <h6 class="mb-1 text-sm leading-normal font-bold text-lime-500">Confirmed</h6>
                                @elseif ($pemesanan->status == 2)
                                <h6 class="mb-1 text-sm leading-normal font-bold text-lime-300">Ongoing</h6>
                                @else
                                <h6 class="mb-1 text-sm leading-normal font-bold text-blue-500">Done</h6>
                                @endif
                            </div>
                            <div class="mb-4 flex gap-2 w-1/2">
                                <label for="catatan" class="block text-gray-700 text-lg font-bold mb-2">Catatan:</label>
                                <label for="nama" class="block text-gray-700 text-lg mb-2"> {{$pemesanan->catatan}}</label>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        @if ($pemesanan->status != 0 && $pemesanan->status != 1)
            ...
            <div id="dokumentasi" class="max-h-0 overflow-hidden transition-all duration-500 ease-in-out">
                <div class="p-5 lg:px-8">
                    <div class="flex-auto px-0 pt-0 pb-2 items-center">
                        <form action="{{ route('pemesanan.tambahgambar', $pemesanan->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="mb-4">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                            <div class="flex justify-center">
                                                <form id="deleteForm{{ $index }}" action="{{ route('pemesanan.hapusgambar', $pemesanan->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
